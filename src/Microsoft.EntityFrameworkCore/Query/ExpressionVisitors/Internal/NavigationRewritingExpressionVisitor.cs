    ...
    public class NavigationRewritingExpressionVisitor : RelinqExpressionVisitor
    {
        ...
        private Expression TryCreateForeignKeyMemberAccess(string propertyName, Expression declaringExpression, INavigation navigation)
        {
            ...
            if (_insideOrderBy)
            {
                ...
                if (qsre == null)
                {
                    var methodExpression = declaringExpression as MethodCallExpression;
                    if (methodExpression != null && EntityQueryModelVisitor.IsPropertyMethod(methodExpression.Method))
                    {
                        qsre = methodExpression.Arguments[0] as QuerySourceReferenceExpression;
                    }
                }
